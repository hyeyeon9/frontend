...
export default function ProductComparisonView({
  ...
  useEffect(() => {
    const fetchProductData = async () => {
      ...
      try {
        ...
        // 선택한 날짜의 데이터
        const currentResponse = await fetchGetDailySalesByDateAndHour(
  ...
  const previousTotal = productData.previousData.reduce(
    (sum, item) => sum + item.totalPrice,
    0
  );
  const currentTotal = productData.currentData.reduce(
    (sum, item) => sum + item.totalPrice,
    0
  );
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div>
          <div className="mb-4">
            <h3 className="text-base font-semibold text-gray-700 mb-2">
              7일 전 ({getPreviousDate(selectedAlert.alertDate)}){" "}
              ...
              <table className="w-full border-collapse">
                <thead className="sticky top-0 bg-gray-100 z-10">
                  <tr className="border-b border-gray-200">
                    <th className="px-4 py-2 text-leftfont-medium text-gray-500">
                      상품명
                    </th>
        ...
        <div>
          <div className="mb-4">
            <h3 className="text-base font-semibold text-gray-700 mb-2">
              현재 ({selectedAlert.alertDate}) {selectedAlert.alertHour}시 판매
              기록
            </h3>
            <p className="text-sm text-gray-500">
              총 판매액:{" "}
              ...
              <table className="w-full border-collapse">
                ...
                <tbody>
                  {productData.currentData.map((product) => {
                    ...
                    // 로그 찍기: 현재 상품 ID와 이전 상품 존재 여부
                    console.log(
                      `현재 상품 ID: ${product.productId}, 이전 상품 존재 여부:`,
                      !!prevProduct
                    );
                        ...
                        <td className="px-4 py-3">
                          {salesChange && (
                            <div className="flex items-center">
                              {salesChange.icon}
                              <span className={`ml-1 ${salesChange.className}`}>
                                {salesChange.text}
                              </span>
                            </div>
                          )}
