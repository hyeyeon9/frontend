    ...
    <>
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="bg-gray-50 border-b border-gray-200">
              ...
              <th className="px-6 py-3 text-left text-sm font-medium text-gray-500 uppercase tracking-wider">날짜</th>
              <th className="px-6 py-3 text-left text-sm font-medium text-gray-500 uppercase tracking-wider">시간</th>
              <th className="px-6 py-3 text-left text-sm font-medium text-gray-500 uppercase tracking-wider">
                이전 매출
              </th>
              <th className="px-6 py-3 text-left text-sm font-medium text-gray-500 uppercase tracking-wider">
              ...
              <th className="px-6 py-3 text-left text-sm font-medium text-gray-500 uppercase tracking-wider">차이</th>
            </tr>
          </thead>
          <tbody>
            {salesData.map((alert, index) => (
              ...
              >
                ...
                <td className="px-6 py-4 whitespace-nowrap text-base text-gray-500">{alert.alertDate}</td>
                <td className="px-6 py-4 whitespace-nowrap text-base text-gray-500">{alert.alertHour}시</td>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm font-semibold">
                  <span
                    className={
                      alert.difference > 0 ? "text-green-600" : alert.difference < 0 ? "text-red-600" : "text-gray-600"
                    }
                  >
                    {Number(alert.difference).toLocaleString()}원 ({formatPercentage(alert.percentageDifference)}%)
                  </span>
                </td>
              </tr>
            ))}
      ...
      {selectedAlert && !onRowClick && (
        <ReportDetailModal
          ...
          toggleProductDetails={() => setShowProductDetails(!showProductDetails)}
