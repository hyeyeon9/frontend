  ...
  return (
    <div className="bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden">
      <div className="bg-blue-50 p-4 border-b border-gray-200">
        <h2 className="text-lg font-semibold text-blue-800 text-center">유통기한 임박</h2>
      </div>
      <div className="p-2">
        {items.length === 0 ? (
          <p className="text-sm text-gray-500 py-4 text-center">임박한 상품이 없습니다.</p>
        ) : (
          <div className="divide-y divide-gray-100">
            {items.slice(0, 3).map((item) => {
              ...
              return (
                <div key={item.batchId} className="flex items-center py-3 px-3 hover:bg-gray-50">
                  <div className="h-10 w-10 flex-shrink-0 bg-gray-100 rounded-md flex items-center justify-center mr-3">
                    <span className="text-xl">{icon}</span>
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="font-medium text-sm md:text-base truncate">{item.goodsName}</div>
                    <div className="text-xs md:text-sm text-red-500">{formatDate(item.expirationDate)}</div>
                    <div className="text-xs md:text-sm text-gray-500">재고: {item.stockQuantity}개</div>
                  </div>
      ...
      {items.length > 0 && (
        <div className="bg-gray-50 p-3 border-t border-gray-200 text-center">
          <Link to="/expiring-items" className="text-sm text-blue-600 hover:text-blue-800 font-medium">
            모든 유통기한 임박 상품 보기
          </Link>
        </div>
      )}
