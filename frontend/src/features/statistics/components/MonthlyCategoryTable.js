...
import { fetchGetMonthlyCategory, fetchGetMonthlySubCategory } from "../api/HttpStatService"
...
export default function MonthlyCategoryTable({ month }) {
  ...
  const fetchSalesData = useCallback(async () => {
    try {
      ...
      if (response.data && response.data.length > 0) {
        ...
        // 최고 판매 카테고리 찾기
        const topCategory = [...response.data].sort((a, b) => b.monthlyPrice - a.monthlyPrice)[0]
  ...
  const calculateCategoryDistribution = () => {
    ...
    // 상위 3개 카테고리 추출
    const top3Categories = [...salesData].sort((a, b) => b.monthlyPrice - a.monthlyPrice).slice(0, 3)

    return top3Categories.map((cat) => ({
      name: getCategoryName(cat.categoryId),
      percentage: ((cat.monthlyPrice / totalSales) * 100).toFixed(1),
    }))
  }
      ...
      {summary.topCategory && (
        <div className="mb-4 grid grid-cols-1 md:grid-cols-3 gap-3">
          <div className="bg-white p-3 rounded-lg border shadow-sm">
            ...
            <div className="text-sm text-gray-700">{formatCurrency(summary.topCategory.monthlyPrice)}</div>
          </div>
      ...
      {summary.topCategory && (
        <div className="mt-4 p-3 bg-blue-50 border border-blue-100 rounded-lg text-sm text-gray-700">
          ...
          <ul className="list-disc pl-5 space-y-1">
            <li>
              <span className="font-medium">{getCategoryName(summary.topCategory.categoryId)}</span> 카테고리가 이번 달
              최고 매출을 기록했습니다. (총 {formatCurrency(summary.topCategory.monthlyPrice)}, 전체 매출의{" "}
              {getPercentOfTotal(summary.topCategory.monthlyPrice)})
            </li>
