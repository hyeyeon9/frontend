...
import { fetchGetDailyCategory, fetchGetDailySubCategory } from "../api/HttpStatService"
...
export default function DailyCategoryTable({ date }) {
  ...
  const fetchSalesData = useCallback(async () => {
    try {
      ...
      // 해당하는 날짜의 데이터 가져오기
      const response = await fetchGetDailyCategory(date)
      ...
      if (response.data && response.data.length > 0) {
        ...
        // 최고 판매 카테고리 찾기
        const topCategory = [...response.data].sort((a, b) => b.dailyPrice - a.dailyPrice)[0]
  ...
  const fetchSubCategoryData = async (categoryId) => {
    try {
      const response = await fetchGetDailySubCategory(date, categoryId)
  ...
  const getPercentOfTotal = (value) => {
    if (!summary.totalSales || summary.totalSales === 0) return "0.00%"
    const percent = (value / summary.totalSales) * 100
  ...
  return (
    <div>
      ...
      {summary.topCategory && (
        <div className="mb-4 grid grid-cols-1 md:grid-cols-3 gap-3">
          <div className="bg-white p-3 rounded-lg border shadow-sm">
            ...
            <div className="text-sm text-gray-700">{formatCurrency(summary.topCategory.dailyPrice)}</div>
          </div>
      ...
      {summary.topCategory && (
        <div className="mt-4 p-3 bg-blue-50 border border-blue-100 rounded-lg text-sm text-gray-700">
          ...
          <ul className="list-disc pl-5 space-y-1">
            <li>
              <span className="font-medium">{getCategoryName(summary.topCategory.categoryId)}</span> 카테고리가 가장
              높은 매출을 기록했습니다. (총 {formatCurrency(summary.topCategory.dailyPrice)}, 전체 매출의{" "}
              {getPercentOfTotal(summary.topCategory.dailyPrice)})
            </li>
            <li>이 카테고리의 상품 재고를 충분히 유지하고 프로모션을 고려해보세요.</li>
      ...
      <SubCategoryModal
        ...
        dataKeys={{ amountKey: "dailyAmount", priceKey: "dailyPrice" }}
