    ...
    try {
      ...
      if (response.data && response.data.length > 0) {
        ...
        const totalSales = response.data.reduce(
          (sum, item) => sum + item.dailyPrice,
          0
        );
        const totalTransactions = response.data.reduce(
          (sum, item) => sum + item.dailyAmount,
          0
        );

        // 최고 판매 시간대 찾기
        const topHour = [...response.data].sort(
    ...
    () => [
      {
        ...
        Cell: ({ value, row }) => {
          const isTopHour =
            summary.topHour && summary.topHour.salesHour === value;
          return (
            <div className="flex items-center justify-center">
              <span
                className={`${
                  isTopHour ? "font-semibold text-indigo-600" : ""
                }`}
              >{`${value}시`}</span>
              {isTopHour && (
      ...
      },
      {
        ...
        Cell: ({ value, row }) => {
          const isTopHour =
            summary.topHour &&
            summary.topHour.salesHour === row.original.salesHour;
          return (
            <div
              className={`text-center ${
                isTopHour ? "text-indigo-600 font-semibold" : ""
              }`}
            >
              {formatAmount(value)}
            </div>
      ...
      {summary.topHour && (
        <div className="mb-4 grid grid-cols-1 md:grid-cols-3 gap-3">
          <div className="bg-white p-3 rounded-lg border shadow-sm">
            ...
            <div className="flex items-center">
              <span className="text-lg font-bold text-indigo-600">
                {summary.topHour.salesHour}시
              </span>
              <span className="ml-2 text-gray-700">
                {formatCurrency(summary.topHour.dailyPrice)}
              </span>
            </div>
          </div>
          ...
          <div className="bg-white p-3 rounded-lg border shadow-sm">
            ...
            <div className="text-lg font-bold text-gray-800">
              {formatAmount(summary.totalTransactions)}회
            </div>
          </div>
        </div>
      )}
