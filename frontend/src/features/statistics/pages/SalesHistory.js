...
function formatDateTo(date) {
  const nativeDate = date instanceof Date ? date : new Date(date); // 여기가 핵심!
...
export default function SalesHistory() {
  ...
  const [pageSize, setPageSize] = useState(20); // 행: 기본 20개
  const [paymentStatus, setPaymentStatus] = useState(1); // 기본: 결제완료
  ...
  }, [formattedDate, currentPage, pageSize]);
    ...
    <div className="bg-gray-50 min-h-screen p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row gap-4">
          {/* 판매 기록 테이블 (60%) */}
          <div className="bg-white rounded-xl shadow-sm p-4 w-full md:w-3/5">
                    ...
                    return (
                      <tr
                        ...
                        className={`hover:bg-gray-50 cursor-pointer ${
                          selectedSale &&
                          selectedSale.ordersId === sale.ordersId
                            ? "bg-indigo-50"
                            : ""
                        }`}
                      >
          ...
          <div className="bg-white rounded-xl shadow-sm p-4 w-full md:w-2/5">
            {selectedSale ? (
              <SalesHistoryDetail sale={selectedSale} />
            ) : (
